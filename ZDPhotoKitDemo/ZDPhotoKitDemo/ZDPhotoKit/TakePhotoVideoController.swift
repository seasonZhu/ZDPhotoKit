//
//  TakePhotoVideoController.swift
//  JiuRongCarERP
//
...
/// 照相拍摄控制器
class TakePhotoVideoController: UIViewController {
    ...
    private lazy var layerView: UIView = {
       let view = UIView(frame: CGRect(x: 0, y: ImageConstant.kNavigationBarHeight, width: ImageConstant.kScreenWidth, height: ImageConstant.kScreenWidth))
    ...
    private lazy var remindLabel: UILabel = {
        let label = UILabel(frame: CGRect(x: 0, y: maskView.bounds.size.height - 20, width: ImageConstant.kScreenWidth, height: 10))
        label.textColor = UIColor.yellow
        label.text = "轻触拍照，长按拍摄"
    ...
    private lazy var focusingView: UIImageView = {
        let imageView = UIImageView(image: UIImage(named:"camera_focusing"))
    ...
    private lazy var timeLabel: UILabel = {
        let label = UILabel(frame: CGRect(x: 0,
                                          y: layerView.frame.origin.y + layerView.frame.size.height,
    ...
    private lazy var squareButton: UIButton = {
        ...
        button.setImage(UIImage(named: "photo_camera_square_normal"), for: .normal)
        button.setImage(UIImage(named: "photo_camera_square_select"), for: .selected)
    ...
    private lazy var rectangleButton: UIButton = {
        let button = UIButton(frame: CGRect(x: squareButton.frame.origin.x + squareButton.frame.size.width,
                                            ...
                                            width: 50, height: 50))
        button.setImage(UIImage(named: "photo_camera_shu_normal"), for: .normal)
        button.setImage(UIImage(named: "photo_camera_shu_select"), for: .selected)
    ...
    private lazy var recordButton: RecordButton = {
        let button = RecordButton(frame: CGRect(x: 0, y: 0, width: 80, height: 80))
        button.center.x = view.center.x
        button.center.y = timeLabel.frame.size.height + timeLabel.frame.origin.y + 50
    ...
    private lazy var deleteButton: UIButton = {
        ...
        button.alpha = 0
        button.setImage(UIImage(named: "video_delete_dustbin"), for: .normal)
    ...
    private lazy var nextButton: UIButton = {
        let button = UIButton(frame: recordButton.frame)
        button.alpha = 0
        button.setImage(UIImage(named: "video_next_button"), for: .normal)
        button.setImage(UIImage(named: "video_next_button_highlighted"), for: .highlighted)
        button.setImage(UIImage(named: "video_next_button_disabled"), for: .disabled)
    ...
    private lazy var rightBarItem: UIBarButtonItem = {
        let image = UIImage(named: "camera_overturn")?.withRenderingMode(.alwaysOriginal)
    ...
    private func setUpCamera() {
        ...
        videoQueue = DispatchQueue(label: "QianQian", attributes: .init(rawValue: 0))
    }
    ...
    private func squareTranfrom() {
        let offsetH: CGFloat = 640.0 / 480.0 * ImageConstant.kScreenWidth - ImageConstant.kScreenWidth
    ...
    private func buttonAnimationClose() {
        ...
        UIView.animate(withDuration: 0.25) {
            self.deleteButton.transform = self.deleteButton.transform.translatedBy(x: self.deleteButton.frame.origin.x - self.deleteButton.frame.size.width / 2.0, y: 0)
            self.nextButton.transform = self.nextButton.transform.translatedBy(x: -(self.nextButton.frame.origin.x + self.nextButton.frame.size.width / 2.0 - ImageConstant.kScreenWidth / 2.0), y: 0)
        }
    }
    ...
    deinit {
        ...
        print("TakePhotoVideoController销毁了")
    }
}

//MARK:- 摄像头相关
extension TakePhotoVideoController {
...
extension TakePhotoVideoController: AVCaptureFileOutputRecordingDelegate {
